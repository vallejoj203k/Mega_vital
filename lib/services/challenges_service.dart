...
class Challenge {
  ...
  final String unit;       // 'kg' | 'reps' | 'seg' | 'km'
  ...
  factory Challenge.fromMap(Map<String, dynamic> m, {double? myRecord, int participantsCount = 0}) =>
      Challenge(
        id:              m['id'] as String,
...
class ChallengesService {
  ...
  Future<List<Challenge>> fetchChallenges() async {
    ...
    try {
      ...
      final results = await Future.wait<dynamic>([
        _db.from('challenge_records')
            .select('challenge_id, user_id, value')
            .inFilter('challenge_id', ids),
      ]);

      final allRecords = results[0] as List;
  ...
  Future<List<ChallengeRecord>> fetchLeaderboard(
    ...
    try {
      ...
      final records = rows.asMap().entries.map((e) {
