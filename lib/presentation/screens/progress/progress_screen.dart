...
class _ProgressScreenState extends State<ProgressScreen>
  ...
  Widget build(BuildContext context) {
    ...
    final userName         = context.read<AuthProvider>().profile?.name ?? 'Usuario';
    final completedSessions = history.where((s) => s.isCompleted).toList();
              ...
              child: Row(
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      Text('Mi Progreso', style: AppTextStyles.headingLarge),
                      Row(children: [
                        ...
                        Icon(
                          ...
                          color: _syncing
                              ? AppColors.textMuted
                              : (_syncOk ? AppColors.primary : AppColors.textMuted),
                        ),
                      ]),
                    ],
                  ),
...
class _DataPoint {
  ...
  final double volume;
  const _DataPoint({required this.date, required this.maxWeight, required this.volume});
}
...
class _SummaryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalVolume   = sessions.fold(0.0, (s, w) => s + w.totalVolume);
    final uniqueEx      = <String>{};
    for (final s in sessions) {
      for (final e in s.exercises) uniqueEx.add(e.exerciseName);
    }

    return Row(
      children: [
        _MiniStat(label: 'Sesiones',      value: totalSessions.toString(),       icon: Icons.fitness_center_rounded, color: AppColors.primary),
        const SizedBox(width: 10),
        _MiniStat(label: 'Ejercicios',    value: uniqueEx.length.toString(),     icon: Icons.sports_gymnastics_rounded, color: AppColors.accentBlue),
        const SizedBox(width: 10),
        _MiniStat(
          label: 'Volumen total',
    ...
    child: DarkCard(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(value, style: AppTextStyles.headingSmall.copyWith(color: Colors.white)),
...
class _ExerciseChart extends StatelessWidget {
  ...
  const _ExerciseChart({required this.data, required this.showVolume, required this.userName, super.key});

  Color get _muscleColor {
    switch (data.muscle) {
      case 'pecho':        return AppColors.accentOrange;
      case 'espalda':
      case 'dorsales':     return AppColors.accentBlue;
      case 'hombros':      return const Color(0xFF80DEEA);
      case 'biceps':
      case 'triceps':      return AppColors.accentPurple;
      case 'piernas':
      case 'cuadriceps':
      case 'isquiotibiales': return const Color(0xFFF48FB1);
      case 'gluteos':      return const Color(0xFFFFCC02);
      case 'abdominales':  return AppColors.accentOrange;
  ...
  Widget build(BuildContext context) {
    ...
    final values  = points.map((p) => showVolume ? p.volume : p.maxWeight).toList();
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Text(data.name, style: AppTextStyles.headingSmall, overflow: TextOverflow.ellipsis),
              ),
              if (values.length >= 2)
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(
                    color: (isUp ? AppColors.primary : AppColors.error).withOpacity(0.12),
                    borderRadius: BorderRadius.circular(8),
                  ),
                  child: Row(
              ...
              const SizedBox(width: 8),
              GestureDetector(
                onTap: () => _showShareSheet(context, color, values, current, best, diff, isUp, unit),
                child: Container(
                  width: 32, height: 32,
                  decoration: BoxDecoration(
                    color: AppColors.surfaceVariant,
                  ...
                  child: const Icon(Icons.share_rounded, color: AppColors.textSecondary, size: 16),
                ),
              ),
            ],
          ),
          ...
          Row(
            children: [
              _ChartStat(label: 'Actual',   value: '${current.toStringAsFixed(1)} $unit'),
              const SizedBox(width: 16),
              _ChartStat(label: 'Mejor',    value: '${best.toStringAsFixed(1)} $unit', highlight: true),
              const SizedBox(width: 16),
              _ChartStat(label: 'Sesiones', value: points.length.toString()),
            ],
          ),
          ...
          if (points.isEmpty)
            ...
            _SinglePointChart(value: values.first, unit: unit, color: color)
          else
            SizedBox(
              height: 130,
              child: CustomPaint(
                painter: _LineChartPainter(
                  values: values, minVal: minVal, maxVal: maxVal,
  ...
  void _showShareSheet(BuildContext context, Color color, List<double> values,
      double current, double best, double diff, bool isUp, String unit) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _ShareSheet(
        ...
        maxWeight:    current,
        bestWeight:   best,
        volume:       data.points.isNotEmpty ? data.points.last.volume : 0.0,
  ...
  String _fmt(DateTime d) => '${d.day}/${d.month}/${d.year.toString().substring(2)}';
}
...
class _ShareSheetState extends State<_ShareSheet> {
  ...
  Future<void> _shareExternal() async {
    ...
    try {
      ...
      final file = File('${dir.path}/progreso_mega_vital.png');
  ...
  Future<void> _postToCommunity() async {
    ...
    try {
      final dir  = await getTemporaryDirectory();
      final file = File('${dir.path}/progreso_comunidad.png');
      await file.writeAsBytes(bytes);

      final content = '¡Revisen mi progreso en ${widget.exerciseName}! '
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text('Compartir progreso', style: AppTextStyles.headingSmall),
          const SizedBox(height: 4),
                ...
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      gradient: _sharing
                          ? null
                          : const LinearGradient(colors: [Color(0xFF00FF87), Color(0xFF00CC6A)]),
                    ...
                    child: Row(
                      ...
                      children: [
                        if (_sharing)
                          ...
                          const Icon(Icons.share_rounded, color: Color(0xFF0A0A0A), size: 18),
                        const SizedBox(width: 8),
                        Text(
                          _sharing ? 'Compartiendo...' : 'Redes sociales',
                          style: AppTextStyles.labelLarge.copyWith(
                            color:    _sharing ? AppColors.textMuted : const Color(0xFF0A0A0A),
                            fontSize: 13,
                          ),
                        ),
                      ],
                ...
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(14),
                      border: Border.all(
                        color: AppColors.accentPurple.withOpacity(_posting ? 0.15 : 0.4),
                      ),
                    ),
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        if (_posting)
                          ...
                          const Icon(Icons.people_rounded, color: AppColors.accentPurple, size: 18),
                        const SizedBox(width: 8),
                        Text(
...
class _ChartStat extends StatelessWidget {
  ...
  const _ChartStat({required this.label, required this.value, this.highlight = false});
...
class _SinglePointChart extends StatelessWidget {
  ...
  final String unit;
  final Color color;
  const _SinglePointChart({required this.value, required this.unit, required this.color});
...
class _LineChartPainter extends CustomPainter {
  ...
  const _LineChartPainter({
    required this.values, required this.minVal, required this.maxVal,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 1; i < values.length; i++) {
      ...
      path.cubicTo(cpX, yOf(values[i - 1]), cpX, yOf(values[i]), xOf(i), yOf(values[i]));
    ...
    for (int i = 0; i < values.length; i++) {
      ...
      if (isLast) {
        canvas.drawCircle(Offset(x, y), 7, Paint()..color = color.withOpacity(0.25));
      }
    ...
    child: Column(
      ...
      children: [
        Container(
          ...
          child: const Icon(Icons.bar_chart_rounded, color: AppColors.textMuted, size: 36),
        ),
        const SizedBox(height: 16),
        Text('Sin datos de progreso', style: AppTextStyles.headingSmall),
        const SizedBox(height: 8),
        Text(
