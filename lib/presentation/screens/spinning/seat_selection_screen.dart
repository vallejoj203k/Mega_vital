...
class _SeatSelectionScreenState extends State<SeatSelectionScreen>
  ...
  void initState() {
    super.initState();
    _selectedSeat = widget.currentSeat;
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(children: [
          ...
          Expanded(child: _buildSeatGrid(tc)),
          _buildBottomBar(tc),
        ]),
      ),
    );
  ...
  Widget _buildClassInfo(AppThemeColors tc) {
    return Container(
      ...
      child: Row(children: [
        ...
        _Divider(color: tc.border),
        _InfoItem(
          icon: Icons.calendar_today_rounded, label: 'Días',
  ...
  Widget _buildLegend(AppThemeColors tc) {
    return Padding(
      ...
      child: Wrap(
        ...
        children: [
          _LegendItem(color: tc.surface, border: _accentColor, label: 'Disponible', secondary: tc.textSecondary),
          _LegendItem(color: _accentColor, border: _accentColor, label: 'Seleccionado', secondary: tc.textSecondary),
          _LegendItem(color: tc.border, border: tc.border, label: 'Ocupado', secondary: tc.textSecondary),
          if (widget.currentSeat != null)
            _LegendItem(
              color: _accentColor.withOpacity(0.2), border: _accentColor,
              label: 'Tu lugar actual', secondary: tc.textSecondary,
      ...
      Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(colors: [
            _accentColor.withOpacity(0.3), _accentColor.withOpacity(0.1)]),
  ...
  Widget _buildSeatGrid(AppThemeColors tc) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Column(
          ...
          return Expanded(
            child: Row(children: [
              ...
              ...List.generate(cols, (col) {
                ...
                final isMySeat   = index == widget.currentSeat;
                final isOccupied = widget.spinClass.reservedSeats.contains(index) && !isMySeat;
                ...
                if (isOccupied) {
                  ...
                  iconData = isMySeat ? Icons.person_rounded : Icons.directions_bike_rounded;
                } else if (isMySeat) {
                  ...
                  child: GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color: borderColor, width: (isSelected || isMySeat) ? 2 : 1),
                        boxShadow: isSelected
                            ? [BoxShadow(color: _accentColor.withOpacity(0.5),
  ...
  Widget _buildBottomBar(AppThemeColors tc) {
    ...
    final seatLabel = hasSelection ? '${_rowLabel(row)}${col + 1}' : '—';
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: hasSelection
                    ? [BoxShadow(color: _accentColor.withOpacity(0.4),
...
class _InfoItem extends StatelessWidget {
  ...
  const _InfoItem({required this.icon, required this.label,
    required this.value, required this.color, required this.muted});
...
class _LegendItem extends StatelessWidget {
  ...
  const _LegendItem({
    ...
    required this.label, required this.secondary,
